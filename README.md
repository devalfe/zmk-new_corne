...
ToDo - rewrite this documentation to depict latest changes.
...
F1 to F12: General function keys for various tasks. The assignment is similar to the num layer numbering with F11, F12 being on the lower movement of the index fingers to avoid any extened pinky actions
...
PRINTSCREEN: Take a screenshot. I usually prest win+shift+S for screenshot but i wanted to have at least one assignement of each key of a full keyboard in some layer.
SCROLLLOCK: Toggle the scroll lock feature.
PAUSE_BREAK: Pause or break functionality.

_Sensor Bindings_

The layer is configured with a scroll_encoder sensor binding, haven't managed to make it work for scrolling yet.
